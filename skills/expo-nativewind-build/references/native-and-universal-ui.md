...
- Use React Native core components plus NativeWind when consistent branding, shared behavior, web support, or one cross-platform implementation matters most.
...
NativeWind styles React Native surfaces. Expo UI components use their documented props and modifiers, so do not assume every NativeWind class works on SwiftUI or Jetpack Compose views. Map the app's semantic colors, spacing, and typography through each API it supports.
